...
function resolveServiceEntry(context: vscode.ExtensionContext): string {
  const config = vscode.workspace.getConfiguration("cursorRemote");
  const configured = config.get<string>("serviceEntryPath", "../server/dist/index.js");
...
function getAutoStartService(): boolean {
  return vscode.workspace.getConfiguration("cursorRemote").get<boolean>("autoStartService", true);
}
...
async function startService(
  ...
  try {
    ...
    if (await checkServiceHealth(serviceUrl)) {
      if (!silent) {
        void vscode.window.showInformationMessage(`Cursor Remote service already running at ${serviceUrl}`);
      }
      return true;
    }
    ...
    child.on("exit", (code) => {
      output.appendLine(`[Cursor Remote] service exited with code ${code ?? 0}`);
      state.serviceProcess = undefined;
    });

    if (!silent) {
      void vscode.window.showInformationMessage("Cursor Remote service started.");
    }
    return true;
  } catch (error) {
    output.appendLine(`[Cursor Remote] startService: ${(error as Error).message}`);
    if (!silent) {
      void vscode.window.showErrorMessage(`Failed to start service: ${(error as Error).message}`);
    }
    return false;
  }
}

async function startPairingNotificationPolling(output: vscode.OutputChannel): Promise<vscode.Disposable> {
  const tick = async (): Promise<void> => {
    ...
    try {
      ...
      if (pairing.devices > state.pairedDevices) {
        void vscode.window.showInformationMessage("Cursor Remote: device paired.");
      }
      state.pairedDevices = pairing.devices;
    } catch (error) {
      output.appendLine(`[Cursor Remote] Pairing poll error: ${(error as Error).message}`);
    }
  };
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("cursorRemote.stopServer", async () => {
      if (!state.serviceProcess) {
        void vscode.window.showInformationMessage("No service process started by this extension.");
        return;
      }
      state.serviceProcess.kill();
      state.serviceProcess = undefined;
      void vscode.window.showInformationMessage("Cursor Remote service stop requested.");
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("cursorRemote.showPairingQr", async () => {
      try {
        const serviceUrl = getServiceUrl();
        const pairing = await getPairing(serviceUrl);
        const qrDataUrl = await QRCode.toDataURL(pairing.payload, { margin: 1, width: 280 });
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("cursorRemote.sendTestPush", async () => {
      try {
        ...
        void vscode.window.showInformationMessage(`Push complete: sent=${result.sent}, failed=${result.failed}`);
      } catch (error) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("cursorRemote.checkCdpConnection", async () => {
      try {
        ...
        void vscode.window.showErrorMessage(`Service CDP check failed: ${detail}`);
      }
    })
  );
}
