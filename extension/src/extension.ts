...
import { PairingServer } from "./services/pairingServer";
...
import { startAgentIdleWatcher } from "./cdp/agentIdleWatcher";
	...
	const server = new PairingServer({
		...
		onDeviceRegistered: (device) => {
			void vscode.window.showInformationMessage(
				`Cursor Remote: device paired (${device.platform}).`,
			);
			if (state.server) {
				void notifyAllPairedDevices(context, state.server, output, {
					title: "Cursor Remote",
					body: `A device paired: ${device.platform}.`,
				});
			}
		},
	});
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
	...
	const config = vscode.workspace.getConfiguration("cursorRemote");
	const notifyOnIdle = config.get<boolean>("notifyOnAgentIdle", true);
	const idlePollMs = config.get<number>("agentIdlePollMs", 8000);

	context.subscriptions.push(
		startAgentIdleWatcher({
			context,
			output,
			getServer: () => state.server,
			pollMs: idlePollMs,
			enabled: notifyOnIdle,
		}),
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("cursorRemote.checkCdpConnection", async () => {
			...
			const cdpUrl = cfg.get<string>("cdpUrl", "http://127.0.0.1:9222");
			try {
