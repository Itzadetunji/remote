...
enum PairingClientError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .httpStatus(let code, let body):
            if code == 401 {
                return "Pairing was rejected. Check that the QR code is current."
            }
    ...
    private static func pairingFriendlyTransportMessage(for error: Error) -> String {
        ...
        case .cannotFindHost, .cannotConnectToHost, .dnsLookupFailed:
            return "Couldn’t connect to that address. Check the Mac is running Cursor Remote, the port isn’t blocked by a firewall, and the QR uses your Mac’s Wi‑Fi IP."
        case .timedOut:
            return "The connection timed out. Move closer to the router or check that the pairing server is still running on your Mac."
        case .networkConnectionLost:
            return "The connection dropped. Stay on Wi‑Fi and try scanning again."
        default:
            break
        }
...
struct PairingClient: Sendable {
    ...
    func registerDevice(
        ...
        let body = DeviceRegistrationBody(deviceToken: deviceTokenHex, platform: "ios")
